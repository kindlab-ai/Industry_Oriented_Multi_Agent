# Hand-written coding notes (April–May 2026)

These are the notes the collection was first assembled with. They are kept
verbatim because they are judgements a generator cannot reproduce, and
because extending them to the rest of the pool would mean inventing
assessments for papers nobody wrote them for.

**Scope and caveats.** They cover 56 of the 125 documents now in the pool. Two of the rows below are wrong about the PDF stored under them — the *Large Language Model Agent: A Survey on Methodology, Applications and Challenges* row names a file whose contents are Xi et al., *The Rise and Potential of Large Language Model Based Agents*, and the scientific-discovery row gives the subtitle *“Orchestrating Scientists and Robots through LLM-based Agents”* where the paper's own subtitle is *“Orchestrating Scientists, Language, Code, and Physics”*. About half the links resolved to a venue's front page rather than to the paper. **`README.md` and `index/papers.csv` supersede this file on every factual field; use these notes for the reading and nothing else.**

The preliminary list also names two papers that were never added to the collection: *Multi-Agent Collaboration via Evolving Orchestration* and *ACC-Collab: An Actor-Critic Approach to Multi-Agent LLM Collaboration*.
